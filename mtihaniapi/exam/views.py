...
def download_cluster_quiz_pdf(request):
    ...
    p.drawString(50, height - 70, f"Cluster: {cluster_id} | Generated by Mtihani App")
...
def get_percentile_performances(request):
    try:
        ...
        top_students = [p for p in performances if p.avg_score is not None and p.avg_score >= percent_90]
        bottom_students = [p for p in performances if p.avg_score is not None and p.avg_score <= percent_10]
